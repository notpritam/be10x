...
**v1 requirement — URL-aware board state.** Every interaction's state — the selected task, the view/team filter, whether the deep-dive is expanded, the review focus — is reflected in the URL, so a page refresh restores the exact same state, a link is shareable, and browser back/forward work. Pairs with live updates: today the board and open task **poll** for agent updates (a stopgap so the human never manually refreshes); v1 should be deep-linkable now and move to a push/subscribe model later. Belongs in the web-board slice (4).

## 16. Deferred (v2+)

Remote & hosting (responsive/phone UI, free Cloudflare Tunnel, host-your-own instance, shared URLs) → **v2**. Deterministic verification (Playwright/Chromium, persistent Chrome profiles, verification recipes feeding self-rating) → **v3**. Ecosystem (more task types, type sharing, external sync) → **v4**. Scale & insight (cross-team analytics, fleet orchestration) → **v5**. **Background merge/verification status tracker** — a toggleable (via dashboard settings), cron-driven check the agent runs itself to keep each task's *verified / in-progress / done / merged* status live on the board without manual updates → rides the **v3** verification work. **GitHub PR integration (v2)** — link a task to its pull request (stored in `refs`), pull the PR's review comments and threads into the task's activity, and resolve or reply to them from within be10x with two-way sync back to GitHub, so a reviewer's GitHub comments and the board stay in lockstep. **Agent-driven generative UI (v3/v4)** — instead of a fixed input widget, the agent specifies the interactive component to render for a request (a JSON schema / sandboxed HTML / iframe / buttons / an MCP action), making approvals and inputs open-ended and agent-driven. **Durable agent work-state / session-independence (v2)** — today an agent's working context (its git worktree, edited files, in-progress diff, research and reasoning) lives in the ephemeral agent session and is lost if that session dies. be10x should OWN this state so agents are **stateless and resumable**: (1) **worktree lifecycle** — each task's git worktree (path · branch · base commit) is tracked on the task (`refs.worktree` is reserved for this) and created/cleaned up by be10x, not the agent; (2) **WIP file/diff snapshots** — the in-progress edits and diff are periodically snapshotted into be10x (per task) so nothing is lost on session death; (3) **code &amp; research persistence** — research dossiers, plans, and code artifacts are the task's durable record (as the chat-v2 seed intended). A stateless **agent runner** reconstructs full context from be10x (task + worktree + last WIP snapshot + research) and resumes — with no dependency on the original agent session. **Ask-a-teammate from any session (v2+, captured 2026-07-03, not designed)** — let an agent in *any* Claude Code session (not just one already tracked as a be10x task) ask a specific teammate for approval/input via an MCP tool, routed through be10x rather than blocking the session. Sketch: the tool posts the request to be10x (auto-adopting the session as a lightweight task via the existing `gfa_import_task` pattern if it isn't one already), tags the named teammate, and the session's turn ends there — no blocking wait. The teammate answers (approve/reject/comment) from their be10x dashboard. Open question, deliberately left unresolved: how the *original* session finds out — three options surfaced so far: (1) the human just relays the answer back into the session manually once they see it on the board (simplest, no new delivery mechanism — current lean); (2) the existing wake-queue + `claude --resume <sessionId>` machinery (already used by `be10x connect`) auto-resumes the exact session once answered; (3) the MCP call blocks/polls until answered (ties up the session for however long the person takes). Needs real design before building — this entry is a placeholder so the idea isn't lost, not a spec.
