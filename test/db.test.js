...
test('openDb applies the schema and enforces foreign keys', () => {
  ...
  const tables = db
    ...
    .all()
    .map((r) => r.name);
  assert.deepEqual(tables, ['bug_events', 'bug_shares', 'bugs', 'comments', 'device_codes', 'input_requests', 'memberships', 'notifications', 'plan_versions', 'projects', 'reviews', 'run_steps', 'runs', 'sessions', 'share_links', 'task_events', 'tasks', 'teams', 'telemetry_events', 'tokens', 'users', 'wake_queue']);
