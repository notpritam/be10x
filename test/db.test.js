...
test('openDb applies the schema and enforces foreign keys', () => {
  ...
  const tables = db
    ...
    .all()
    .map((r) => r.name);
  assert.deepEqual(tables, ['comments', 'device_codes', 'input_requests', 'memberships', 'plan_versions', 'projects', 'reviews', 'run_steps', 'runs', 'sessions', 'share_links', 'task_events', 'tasks', 'teams', 'tokens', 'users', 'wake_queue']);
