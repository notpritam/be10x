...
// Regression test for a real production incident: every test above calls migrate() directly against
// a hand-built old-shape db, which never exercises the actual boot sequence (db.exec(schema.sql)
// FIRST, migrate() second) that openDb() runs. schema.sql's CREATE TABLE IF NOT EXISTS is a no-op
// against an existing old-shape `projects` table — so any statement in schema.sql that references
// projects.team_id/owner_id (an index, say) would run against the OLD columns and crash with
// "no such column: team_id" before migrate() ever got a chance to add them. This must go through
// the REAL openDb() on a REAL on-disk file (an in-memory db can't be closed and reopened the way
// production's persistent disk is) to actually catch that class of bug.
test('openDb() self-heals a real on-disk old-shape database end to end, not just migrate() in isolation', () => {
  const dir = mkdtempSync(join(tmpdir(), 'gfa-olddb-'));
  ...
  try {
    ...
    seed.exec(`
      CREATE TABLE users (id TEXT PRIMARY KEY, email TEXT NOT NULL UNIQUE, display_name TEXT NOT NULL, password_hash TEXT NOT NULL, created_at INTEGER NOT NULL);
      CREATE TABLE teams (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, bias_md TEXT NOT NULL DEFAULT '', created_by TEXT NOT NULL, created_at INTEGER NOT NULL);
      CREATE TABLE memberships (id TEXT PRIMARY KEY, team_id TEXT NOT NULL, user_id TEXT NOT NULL, role TEXT NOT NULL, created_at INTEGER NOT NULL, UNIQUE(team_id, user_id));
      CREATE TABLE projects (id TEXT PRIMARY KEY, key TEXT NOT NULL UNIQUE, name TEXT NOT NULL, default_branch TEXT, root_path TEXT, created_at INTEGER NOT NULL);
      CREATE TABLE tasks (
        id TEXT PRIMARY KEY, human_id TEXT NOT NULL UNIQUE, type TEXT NOT NULL, scope TEXT NOT NULL, team_id TEXT, project_id TEXT,
        owner_id TEXT NOT NULL, assignee_id TEXT, reviewer_id TEXT, title TEXT NOT NULL, status TEXT NOT NULL, severity TEXT NOT NULL DEFAULT 'medium',
      ...
      CREATE TABLE runs (id TEXT PRIMARY KEY, task_id TEXT NOT NULL, project_id TEXT, session_id TEXT, executor TEXT NOT NULL DEFAULT 'claude', model TEXT,
        worktree_path TEXT, branch TEXT, base_ref TEXT, status TEXT NOT NULL DEFAULT 'starting', pid INTEGER, result_json TEXT, error TEXT,
    ...
    assert.doesNotThrow(() => {
      db = openDb(path);
    }, 'openDb() must not crash booting against a real old-shape production-like database');
    const cols = db.prepare('PRAGMA table_info(projects)').all().map((c) => c.name);
    assert.ok(cols.includes('owner_id'));
    assert.ok(cols.includes('team_id'));
    db.close();
